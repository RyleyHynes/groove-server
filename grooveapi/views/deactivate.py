...
from grooveapi.models import Deactivate
...
class DeactivateView(ViewSet):
    ...
    def create(self, request):
        ...
        """
        user = GrooveUser.objects.get(user=request.auth.user)

        deactivate = Deactivate.objects.create(
            deactivatedUser=GrooveUser.objects.get(pk=request.data["deactivatedUser"]),
            approveUser=user
        )
